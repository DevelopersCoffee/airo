...
class LocalBillSplitStorage implements BillSplitStorageService {
  ...
  Future<void> saveSplitHistory(SplitResult result) async {
    try {
      ...
      debugPrint('Saved split: ${result.shareableMessage.substring(0, 50)}...');
    } catch (e) {
  ...
  Map<String, dynamic> _splitResultToJson(SplitResult result) {
    return {
      ...
      'shareableMessage': result.shareableMessage,
      'createdAt': DateTime.now().toIso8601String(),
    };
  }

  SplitResult _splitResultFromJson(Map<String, dynamic> json) {
    final splits = (json['splits'] as List).map((s) => ParticipantSplit(
    ...
    return SplitResult(
      ...
      splits: splits,
      shareableMessage: json['shareableMessage'] ?? '',
    );
  }
