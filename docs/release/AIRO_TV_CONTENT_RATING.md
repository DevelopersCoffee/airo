# Aika Stream Content Rating Worksheet

Console-ready worksheet for Aika Stream content rating submissions. Use this when
...
Do not market Aika Stream as child-directed. Do not select a lower age target
...
| --- | --- | --- |
| User-generated content | No social or in-app user-generated content platform | Users provide private playlist/stream URLs; Aika Stream does not publish, host, or share user content. |
...
| Violence in app UI | No | Aika Stream UI does not include violent content. User-loaded streams are outside app-provided content. |
| Sexual content or nudity in app UI | No | Aika Stream UI does not include sexual content or nudity. |
| Profanity or crude humor in app UI | No | Aika Stream UI does not include profanity or crude humor. |
...
| Content disclaimer | Yes | Aika Stream is a media player only and does not provide channels, playlists, streams, or subscriptions. |
...
```text
Aika Stream is a media player for user-provided playlist and stream URLs. The
