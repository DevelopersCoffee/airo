...
- Use semantic product release tags for the current product line. Historic
  per-product tags use `airo-tv-v0.0.x` for Aika Stream and `v0.0.x` for the full
  app.
- Since `v0.0.6-rc.1`, the release orchestrator publishes **one aggregate
  ...
  built — Aika Stream, the full app, Airo Coins, and macOS TV. `airo-tv-v*` tags
  are retained for history and are no longer minted per wave. Anything that
  resolves "the current Aika Stream release" must therefore match on the published
