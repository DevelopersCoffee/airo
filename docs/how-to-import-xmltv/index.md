...
title: "How to Import an XMLTV Guide into Aika Stream"
description: "Add an authorized XMLTV guide to Aika Stream alongside your M3U/M3U8 playlist for programme data and clearer playback diagnostics."
eyebrow: "How-to"
...
steps:
  - "Open Aika Stream and choose the playlist or source action — the same screen used for playlists."
...
structured_data:
  ...
  "@type": HowTo
  name: How to Import an XMLTV Guide into Aika Stream
  description: Add an authorized XMLTV guide source to Aika Stream alongside an existing playlist.
  step:
    - "@type": HowToStep
      text: Open Aika Stream and choose the playlist or source action — the same screen used for playlists.
    - "@type": HowToStep
...
faq:
  - q: "The guide loaded but shows no programme data. What's wrong?"
    a: "Two common causes: confirm the Aika Stream build you're running includes guide support, and confirm the XMLTV feed's channel identifiers actually match the channels in your playlist — a guide with no matching IDs will load but show nothing."
  - q: "Do I need an XMLTV guide to use Aika Stream?"
    a: "No. Playlist playback, search, and favorites all work without one. The guide is an addition, not a requirement."
  - q: "Does Aika Stream provide XMLTV guide data?"
    a: "No. Aika Stream ships no EPG data of its own — you supply a guide URL from a provider you're already authorized to use, the same boundary that applies to playlists."
---
...
playlist, Aika Stream has a guide with nothing to attach it to. Check that
