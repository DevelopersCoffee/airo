...
2. Match schedule, meeting, agenda, appointment, and calendar phrases to the
   local calendar skill. That skill reads the on-device calendar store and
   summarizes events locally.
3. Execute a matching local tool only when that tool belongs to the current
   build and release profile.
4. Use AI generation for general prompts that do not map to a local tool.

## Starting Chat

Chat no longer opens a catalog of model families. **Automatic** selects an
installed chat-capable model, or prompts **Install recommended** for this
device. Customize a slot only when you want to override Automatic. Browse
...
**Scribe | Assistant | Intelligence | Wellbeing**. Intelligence is a control
center, not a fourth super-app tab. From the phone super-app, open it from the
**Intelligence** card on Assistant (`/assistant/models`).

Inside Intelligence:

- **Overview** — job cards (Chat, Scribe, Documents, Vision when that job has
  an installable model). A **Recommended setup** button installs the ranked
  pipeline when something is missing.
- **Capabilities** — Automatic by default, with **Why?** for selection
  rationale (memory, language, task fit). That is not the same as **Runtime**,
  which opens the Health Center.
- **Models** — the Intelligent Model Manager, grouped by capability, with
  badges, used-by, and technical details.
- **Library** — browse and install, including public GGUF. Community packages
  stay here, not on Overview.
- **Diagnostics** — device capability report and runtime health.

Layout follows width: a left rail at 900px and wider, a chip row on tablet, and
an Overview-first phone screen with Advanced push routes. Airo does not show a
Vision install card when no vision package exists for the device.
...
availability claim for a public Airo release. In Intelligence it is the
**Models** section, not the first screen.
...
LiteRT-LM startup is shown as ready only when Airo has a verified downloaded
package path or an explicit `LITERT_LM_MODEL_PATH` pointing at a local
artifact. A configured download URL is treated as an install source, not as
proof that the model is already runnable on the device.
...
- chat-entity pending answers for insurance claims, hospital/surgery recovery,
  and under-construction property purchases, using only facts already stored
  on the local graph (no invented clinical or legal advice)

Pinned Hospital Recovery and Property Purchase assistants read the chat entity
graph first, then LifeTrack if a journey was saved. A hospital bill can sit on
both a surgery journey and an insurance claim at once.
...
Open Intelligence from:

- Assistant hub card: **Intelligence**
- Route: `/assistant/models`
...
In the standalone Airo Mind app, **Intelligence** ranks Meeting Assistant
speech automatically. Named strategies are **Recommended**, **Long meetings**,
**Indian languages**, and **English only** — the UI does not ask you to pick
Whisper Tiny versus Small by id. Generation mode, when shown, is Auto /
Standard / Enhanced Indic.

The manager still lists on-device scribe weights (speech + compact writing
