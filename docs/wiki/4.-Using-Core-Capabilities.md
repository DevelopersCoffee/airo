...
segments enter these stages.

The full app now has injectable local-provider seams for text embeddings and
anonymous speaker clustering. Embedding results retain exact model identity and
use existing local meeting storage. Speaker results represent anonymous time
ranges, including overlap, without claiming a real-world identity. Neither
provider is selected automatically: until an approved installed model or
qualified local diarization provider is opened and injected, its stage remains
unavailable. Memory updates also remain unavailable pending the approved Airo
Mind runtime.
