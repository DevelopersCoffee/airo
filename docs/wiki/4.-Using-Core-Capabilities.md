...
- **Live recording.** Record from Scribe or Notes. Capture still goes through
  ...
  created with transcript, summary, and key points. Desktop Preview can also
  show a live transcript and Conversation IR insights while you record.
- **Live preview settings (desktop).** Profile → Meeting recordings: **Live
  insights**, **Expand insights by default**, and **Live intelligence**
  (Automatic / Prefer full insights / Transcript only). Prefer full keeps
  insights on Warm/Low battery; critical heat or battery still drops to
  capture + STT. Transcript only hides insights; the file still records.
  These tiles are hidden on phone, tablet, and web. Live capture remains
  Preview — not production qualified.
...
Ask the Assistant **Check my schedule for today**, tap the Calendar prompt
chip, or open **Calendar** from the Assistant hub (`/mind/calendar`).

Airo lists events from every calendar already connected in the device Calendar
app — Apple Calendar, Google, Outlook, and any other account the operating
system has synced. It does not ask whether you mean work or personal, and it
does not send event titles, attendees, locations, or descriptions to a cloud
model.

On first use, tap **Allow Calendar Access**. If access was previously denied,
enable Calendar for Airo in system settings. Empty days say you have no events;
Airo does not invent them. Windows, Linux, and web hosts report that calendar
access is not supported yet.
