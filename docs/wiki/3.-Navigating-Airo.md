...
- `/mind`, `/mind/chat`, and `/mind/models` use the shared shell
  header.
- `/mind/profile` and `/mind/notifications` use their own contextual
  app bars.
...
Ways to Watch also lists **Cast MultiView**, a phone remote for a TV's
MultiView grid (add or remove channels, focus a tile's audio, save and
relaunch layouts) that is separate from casting a single channel. It always
opens — it doesn't wait on device discovery — but currently has no TV to
connect to: the underlying receiver hasn't shipped yet, so it stays on "Not
connected."
...
Aika Stream changelog. Row settings can show or hide Channel, Stats, Filters,
...
**Under qualification (not in the current Aika Stream 0.0.6 release):** the Share
action is designed to copy a versioned Aika Stream link instead of the playlist's
...
Labels and icons for IPTV sections come from the shared
`iptvSettingsSections` manifest. The TV rail iterates that list; the phone
hub stays a hardcoded layout so compact Aika Stream can opt in to Privacy
without giving the super-app a destructive wipe tile.

TV rail order is Theme, Accessibility, Playback, Country, Sources, Privacy,
About. Accessibility is text size (channel names in the library) and captions
Off/On — captions reapply a language only after one was picked in the player.
Country on TV uses the same long-list picker as browse filters.

Phone hub:

- Super-app (`ShellId.mobile`): Appearance, Accessibility, Audio, Playback,
  Country, Playlist Source, EPG Guide Source. No Privacy.
- Compact Aika Stream (`ShellId.tv`): Appearance, Accessibility, Playback,
  Country, Playlist Source, EPG Guide Source, Privacy. No Audio Settings —
  Smart Audio is a phone/tablet concept.

Section ownership follows the modular-app model: Playback, Sources, Country,
and Accessibility live in `feature_iptv`. Theme stays app-level (shared
chrome). Audio stays super-app only (`audio_context_provider` is shared with
Music). Privacy delete-local-data stays app-level because it spans sources,
guide, favorites, and history; compact Aika Stream hosts it without the
streaming-telemetry rows. A setting only lives at the app level when it's
actually cross-app — otherwise it's local to the module that owns it.
