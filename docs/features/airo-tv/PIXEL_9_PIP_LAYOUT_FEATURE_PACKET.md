...
**Layer:** Mixed — Aika Stream mobile presentation, shared rail sizing, and its
Aika Stream Android manifest declaration

## Critical Agent Gate

**Problem:** On a Pixel 9, Android PiP shrinks the complete Aika Stream activity,
including the Aika Stream app bar and its actions, instead of presenting only the
...
**User / actor:** A mobile Aika Stream viewer who backgrounds a live channel into
PiP.
...
from its card geometry, and the Aika Stream Android manifest declares that its
...
**Permissions:** No new runtime permission. The Aika Stream activity declares its
...
smoke test confirms the Aika Stream package is installed and launched; a shared
...
**Happy path:** The screen switches to the video-only player; the Aika Stream app
...
**Actor:** Mobile Aika Stream viewer.
**Preconditions:** At least one channel is available.
**Trigger:** Viewer opens Aika Stream outside PiP.
