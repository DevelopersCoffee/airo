# Aika Stream Device Guide

Last reviewed: 2026-07-16

Aika Stream is a bring-your-own-content IPTV and personal media player. It does not
...
| Desktop / macOS builds | Use only where an Aika Stream desktop artifact is explicitly published for the release line. Feature behavior can differ from TV builds. | Release-dependent |
...
1. Install the Aika Stream build from the approved release channel for the device.
2. Open Aika Stream and confirm that no channels appear until you add your own
   authorized source.
...
1. Install the Fire TV-compatible Aika Stream APK only from a trusted release source.
...
2. Start Aika Stream from the sender build that includes Cast controls.
...
Aika Stream supports bring-your-own playlists. The expected user input is an
authorized M3U source. Aika Stream should never imply that it provides channels,
...
Aika Stream does not bundle a VPN. If a user chooses to use a VPN, it can affect
...
milestone, `v2.0.0.1 - Aika Stream Platform Hardening`, because they are user-facing
Aika Stream capability gaps. Performance-only work remains in the separate Aika Stream
