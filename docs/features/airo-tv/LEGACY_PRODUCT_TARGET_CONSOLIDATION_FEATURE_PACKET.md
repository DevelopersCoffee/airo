...
**Open questions:** None. Pixel 9 is supported by Aika Stream: `configureTvSystemChrome` leaves orientation unrestricted unless the detected form factor is a TV. The standalone debug launcher is removed in this change because parity is established below.

**Decision:** Ready.

### Cross-Agent Contract

**Provider agent:** Aika Stream Flutter Architect / Media Intelligence Architect
**Consumer agent:** Release and DevOps tooling
**Interface/API:** Kept entrypoints and release profiles
**Input shape:** `APP_VARIANT=full` or `APP_VARIANT=tv`; Aika Stream uses `APP_PLATFORM=androidTv`.
**Output shape:** Only `io.airo.app` / `Airo` and `io.airo.app.tv` / `Aika Stream` are product Android package targets.
...
**Persistence:** Existing IPTV settings remain owned by shared providers and are consumed by Aika Stream.
...
**Tests required:** Build-profile contract, `core_release` matrix/Firebase/preflight tests, Aika Stream startup orientation and debug-source tests, and debug APK builds for full and TV targets.
...
#### UC-002: Aika Stream remains the IPTV product on phone and TV

**Actor:** Aika Stream user
**Preconditions:** Aika Stream starts with shared IPTV providers.
...
**Failure paths:** Firebase or optional media-session initialization failure does not prevent Aika Stream from starting.
...
**Then:** Only full Airo and Aika Stream are accepted Android product targets; legacy IPTV/Streaming package/client/profile expectations are absent.
...
- **Tests:** Existing focused release and Aika Stream tests.
...
| Legacy behavior | Aika Stream owner | Evidence |
| --- | --- | --- |
...
| Phone/tablet compact layout | Aika Stream router | `tv_router.dart` falls back to `IPTVScreen` |
| macOS and physical-TV layouts | Aika Stream app/router | shared Aika Stream shell and device-form-factor routing |
| TV remote layout | Aika Stream + `feature_iptv` | TV router, remote-control packages, and existing tests |
| Diagnostics and optional media session | Aika Stream startup | `main_tv.dart` startup/error handling and TV media session integration |
