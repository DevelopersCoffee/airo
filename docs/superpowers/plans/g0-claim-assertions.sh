...
assert present A04 SEC-32  'aggregate.rs' 'nfc\|NFC\|is_control'                  'subject ids NFC-normalized, control chars rejected'
...
assert absent  A10 RA-24   '*.rs'         'format!("{b:02x}")'                    'no per-byte format! (350.82ms vs 16.55ms)'
...
assert absent  A16 SEC-38  'aggregate.rs' 'verify_against'                        'restore does not re-implement admission'
assert present A17 SEC-35  'package.rs'   'push_len_prefixed(&mut aad, &self.nonce)' 'the package nonce is authenticated'
assert present A18 SEC-36  'package.rs'   'LedgerHead\|head_epoch:'               'head_epoch is carried, not re-derived'
