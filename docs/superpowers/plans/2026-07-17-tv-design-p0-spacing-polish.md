# Aika Stream Design Revamp — P0 Spacing/Alignment Polish Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Tighten spacing, alignment, and typographic rhythm across the Aika Stream browse screen and sidebar to an 8-point grid, without changing any screen's structure, navigation behavior, or D-pad focus flow.
...
git commit -m "fix(app): tighten Aika Stream sidebar to a consistent 24/56/28/16/24 spacing rhythm"
```
...
disown
```
Wait for `✓ Built build/macos/Build/Products/Debug/Aika Stream.app` to appear in the log (poll with `tail -5 /tmp/airo_tv_p0_verify.log`), typically 60-120 seconds.

- [ ] **Step 5: Activate, resize, and screenshot the window**

```bash
osascript -e 'tell application "Aika Stream" to activate'
sleep 1.5
osascript -e '
tell application "System Events"
  tell process "Aika Stream"
