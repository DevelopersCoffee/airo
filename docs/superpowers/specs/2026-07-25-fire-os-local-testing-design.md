...
Get Aika Stream running on a physical Fire TV Stick over the local network for manual
...
- Aika Stream already builds as a standard Android leanback app — Fire OS runs stock
...
- TV flavor exists: `APP_VARIANT=tv` dart-define, applicationId `io.airo.app.tv`,
  label "Aika Stream" (`app/android/app/build.gradle.kts`).
