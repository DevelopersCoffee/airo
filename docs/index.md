# 🚀 Airo Super App

**On-device AI platform for PDF/image/audio processing**

An advanced Flutter super app that brings powerful AI capabilities to your device with Gemma 1B (int4 quantized), LiteRT/AI Edge SDKs, and RAG + function calling.

---

## ✨ Features

### 🎯 Core Capabilities
- **PDF/Image Processing** - Extract and analyze documents
- **Audio Processing** - Process audio files on-device
- **AI-Powered** - Gemma 1B with on-device inference
- **Offline First** - Works without internet connection
- **Secure** - SQLCipher encryption for data

### 📱 Platforms
- **Android** - Pixel 9 and compatible devices
- **iOS** - iOS 26+
- **Web** - Chrome and modern browsers
- **Windows** - Desktop application
- **Linux** - Desktop application

### 🎮 Features
- **Quest** - Upload files and ask AI questions
- **Money** - Track transactions and expenses
- **Beats** - Music streaming
- **Loot** - Deals and offers
- **Chess** - Play chess games
- **Reminders** - Smart notifications

---

## 🚀 Quick Start

### For Users
1. Download the latest release from [Releases Page](https://github.com/DevelopersCoffee/airo/releases)
2. Install on your device
3. Start using Airo!

### For Developers
1. [Getting Started Guide](./getting-started/START_HERE.md)
2. [Quick Reference](./getting-started/QUICK_REFERENCE.md)
3. [Development Setup](./getting-started/DEPLOYMENT_COMPLETE.md)

---

## 📚 Documentation

### Getting Started
- [Start Here](./getting-started/START_HERE.md) - New to Airo?
- [Quick Reference](./getting-started/QUICK_REFERENCE.md) - Common commands
- [Deployment Guide](./getting-started/DEPLOYMENT_COMPLETE.md) - Deploy to production

### CI/CD & Releases
- [CI/CD Setup](./setup/CI_CD_SETUP.md) - Configure CI/CD
- [Release Guide](./guides/RELEASE_GUIDE.md) - Create releases
- [CI/CD Complete](./setup/CI_CD_COMPLETE.md) - Full pipeline overview

### Security & Quality
- [Security Setup](./setup/SONAR_SNYK_SETUP_GUIDE.md) - SonarQube & Snyk
- [Security Checklist](./security/SECURITY_CHECKLIST.md) - Security verification
- [Code Quality](./integration/SONAR_SNYK_INTEGRATION.md) - Quality metrics

### Architecture
- [Technical Architecture](./architecture/TECHNICAL_ARCHITECTURE.md) - System design
- [AI Integration](./architecture/GEMINI_NANO_FIX_SUMMARY.md) - Gemini Nano setup

### Features
- [Money Feature](./features/MONEY_FEATURE_COMPLETE.md) - Transaction tracking
- [Chess Feature](./features/CHESS_IMPLEMENTATION_COMPLETE.md) - Chess game
- [Chess Testing](./features/CHESS_TESTING_GUIDE.md) - Testing guide

---

## 🎯 Key Metrics

| Metric | Target | Status |
|--------|--------|--------|
| Offline Accuracy | 90% | ✅ |
| PDF Extraction | <3s | ✅ |
| App Footprint | <1.2GB | ✅ |
| Extraction F1 Score | ≥0.9 | ✅ |
| Battery Usage | <5% per workflow | ✅ |

---

## 🔗 Links

### Repository
- **GitHub**: https://github.com/DevelopersCoffee/airo
- **Releases**: https://github.com/DevelopersCoffee/airo/releases
- **Issues**: https://github.com/DevelopersCoffee/airo/issues

### Dashboards
- **SonarCloud**: https://sonarcloud.io/projects
- **Snyk**: https://app.snyk.io/org/ucguy4u/
- **GitHub Actions**: https://github.com/DevelopersCoffee/airo/actions

### External
- **Flutter**: https://flutter.dev
- **GitHub Pages**: https://pages.github.com
- **SonarCloud**: https://sonarcloud.io
- **Snyk**: https://snyk.io

---

## 📊 Project Status

| Component | Status | Details |
|-----------|--------|---------|
| Core App | ✅ Complete | All features implemented |
| CI/CD Pipeline | ✅ Complete | Multi-platform builds |
| Security Scanning | ✅ Complete | SonarQube & Snyk |
| Documentation | ✅ Complete | Comprehensive guides |
| Releases | ✅ Active | Automated releases |

---

## 🤝 Contributing

We welcome contributions! Please:

1. Fork the repository
2. Create a feature branch
3. Make your changes
4. Submit a pull request

See [Contributing Guide](./getting-started/START_HERE.md) for details.

---

## 📞 Support

### Need Help?

1. **Check Documentation** - Most answers are here
2. **Search Issues** - Your question might be answered
3. **Create an Issue** - If you can't find the answer
4. **Contact Team** - For urgent matters

---

## 📄 License

This project is licensed under the MIT License.

---

## 🎉 Get Started

**Ready to dive in?**

→ [Start with Getting Started Guide](./getting-started/START_HERE.md)

---

**Last Updated**: November 2, 2025
**Version**: 1.0.0
**Status**: ✅ Production Ready

