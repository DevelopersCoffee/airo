...
class _FakeMediaPipeWebClient implements MediaPipeWebClient {
  _FakeMediaPipeWebClient({this.webGpuSupported = true});
...
void main() {
  group('MediaPipeWebRuntimeAdapter', () {
    ...
    test(
      'supportsModel is true only for catalog entries flagged web-capable',
      () async {
        final adapter = MediaPipeWebRuntimeAdapter(
          client: _FakeMediaPipeWebClient(),
        );
    ...
    test(
      'capabilitiesForModel advertises webgpu backend when supported',
      () async {
        final adapter = MediaPipeWebRuntimeAdapter(
    ...
    test(
      'capabilitiesForModel omits gpu backend when WebGPU unsupported',
      () async {
        final adapter = MediaPipeWebRuntimeAdapter(
        ...
        expect(
          capabilities.supportedBackends,
          isNot(contains(RuntimeBackend.gpu)),
        );
        expect(capabilities.supportedBackends, contains(RuntimeBackend.cpu));
      },
    );
    ...
    test(
      'generateText loads the model then returns the client response',
      () async {
    ...
    test(
      'isAvailable is true for a client that can report readiness',
      () async {
        final adapter = MediaPipeWebRuntimeAdapter(
          client: _FakeMediaPipeWebClient(),
        );
