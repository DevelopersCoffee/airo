...
void main() {
  group('paging', () {
    test(
      'loadInitial fetches count and exactly one page, never the whole log',
      () async {
    ...
    test(
      'loadMore fetches the next page and appends, without re-fetching what is loaded',
      () async {
        final port = FakeOperationLogPort(opCount: 12481);
        final controller = RuntimeConsoleController(log: port, pageSize: 50);
        await controller.loadInitial();
    ...
    test(
      'hasMore is false once every row up to totalCount is loaded',
      () async {
  ...
  group('sorting', () {
    test(
      'sortBy sequence ascending reorders the loaded page without a re-fetch',
      () async {
        ...
        controller.sortBy(
          RuntimeConsoleSortField.sequence,
        ); // flip to ascending

        expect(
          controller.rows.first.sequence,
          lessThan(controller.rows.last.sequence),
        );
        expect(
          port.rangeCalls.length,
          1,
          reason: 'sorting must not trigger a fetch',
        );
      },
    );
    ...
    test('sortBy a new field defaults to descending', () async {
      ...
      expect(
        devices,
        orderedEquals([...devices]..sort((a, b) => b.compareTo(a))),
      );
    });
  ...
  group('per-row signature verification', () {
    ...
    test(
      'a signature that fails to verify renders as unverified, not "assumed"',
      () async {
        final port = FakeOperationLogPort(opCount: 50);
        final controller = RuntimeConsoleController(log: port);
        await controller.loadInitial();
        final op = controller.rows.firstWhere(
          (o) => o.signature == SignatureState.verified,
        );
        port.verifyResults[op.sequence] = SignatureState.unverified;
    ...
    test(
      'a verify() port error is captured, not thrown out of the controller',
      () async {
        final port = FakeOperationLogPort(opCount: 50);
        final controller = RuntimeConsoleController(log: port);
        await controller.loadInitial();
    ...
    test(
      'signatureFor falls back to the log-stored state before any verify',
      () async {
        final port = FakeOperationLogPort(opCount: 50);
        final controller = RuntimeConsoleController(log: port);
        await controller.loadInitial();
  ...
  group('replay from a row', () {
    test(
      'replayFrom drives progress from the port as a number, not a spinner flag',
      () async {
        final port = FakeOperationLogPort(opCount: 50);
        final controller = RuntimeConsoleController(log: port);
        await controller.loadInitial();
        ...
        expect(
          controller.isReplaying(op.sequence),
          isFalse,
    ...
    test(
      'mid-replay, isReplaying is true and progress reflects the latest fraction',
      () async {
        final port = FakeOperationLogPort(opCount: 50);
        final controller = RuntimeConsoleController(log: port);
        await controller.loadInitial();
    ...
    test(
      'replayFrom(sequence) is the sequence right-clicked, not sequence 0 or the top row',
      () async {
        final port = FakeOperationLogPort(opCount: 500);
        final controller = RuntimeConsoleController(log: port);
        await controller.loadInitial();
    ...
    test(
      'a replay that fails is captured, not thrown, and clears progress',
      () async {
        final port = FakeOperationLogPort(opCount: 50);
        final controller = RuntimeConsoleController(log: port);
        await controller.loadInitial();
    ...
    test(
      'dismissReplay clears a finished replay back to resting state',
      () async {
        final port = FakeOperationLogPort(opCount: 50);
        final controller = RuntimeConsoleController(log: port);
        await controller.loadInitial();
