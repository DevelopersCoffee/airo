...
void main() {
  ...
  test(
    'a deep link written before Phase 3 claimed /mind for the hub is '
    'migrated',
    () {
      // A notification scheduled by an older build (pointing at the hub's
      // pre-Phase-3 home) fires against this one. It must land on the hub's
      // current home, not the route the router now redirects away from.
      expect(
      ...
      expect(routeFromNotificationPayload('/assistant'), '/mind');
      expect(routeFromNotificationPayload('/assistant/chat'), '/mind/chat');
    },
  );
