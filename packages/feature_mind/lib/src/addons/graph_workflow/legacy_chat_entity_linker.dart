...
class LegacyChatEntityLinker {
  const LegacyChatEntityLinker({
  ...
  ChatEntityGraph ingestInsuranceDomain(ChatEntityGraph graph, String text) {
    if (text.trim().isEmpty) return graph;
    return _ingestDomain(
      graph,
      text,
  ...
  ChatEntityGraph ingestHospitalDomain(ChatEntityGraph graph, String text) {
    if (text.trim().isEmpty) return graph;
    return _ingestDomain(
      graph,
      text,
  ...
  ChatEntityGraph ingestPropertyDomain(ChatEntityGraph graph, String text) {
    if (text.trim().isEmpty) return graph;
    return _ingestDomain(
      graph,
      text,
  ...
  ChatEntityGraph ingestGenericMentions(ChatEntityGraph graph, String text) {
    if (text.trim().isEmpty) return graph;
    return _ingestDomain(
      graph,
      text,
  ...
  ChatEntityGraph _ingestDomain(
    ChatEntityGraph graph,
    String text, {
    required bool includeInsurance,
    required bool includeHospital,
    ...
    ChatGraphNode? claim = _findClaimInGraph(graph);
    ChatGraphNode? insurer;
    ChatGraphNode? broker;
    ChatGraphNode? policy;
    ChatGraphNode? documents;
    ChatGraphNode? stay = _findHospitalStayInGraph(graph);
    ChatGraphNode? hospitalOrg = _findHospitalOrgInGraph(graph);
    ChatGraphNode? property = _findPropertyInGraph(graph);
    ...
    if (includeInsurance) {
      ...
      final linkedHospital = hospitalOrg ?? _findHospitalOrgInGraph(graph);
      if (claim != null && linkedHospital != null) {
        link(claim.id, ChatEntityRelation.relatedTo, linkedHospital.id);
      }
    }

    if (includeHospital) {
      claim ??= _findClaimInGraph(graph);
    ...
    if (includeGeneric) {
      claim ??= _findClaimInGraph(graph);
      stay ??= _findHospitalStayInGraph(graph);
      final combinedFacts = {
  ...
  ChatGraphNode? _findClaimInGraph(ChatEntityGraph graph) {
    for (final node in graph.nodes) {
      if (node.type == EntityType.identifier &&
  ...
  ChatGraphNode? _findHospitalStayInGraph(ChatEntityGraph graph) {
    for (final node in graph.nodes) {
      if (node.attributes['kind'] == 'hospital_stay') return node;
    }
    return null;
  }

  ChatGraphNode? _findHospitalOrgInGraph(ChatEntityGraph graph) {
    for (final node in graph.nodes) {
      if (node.type == EntityType.organization &&
          node.attributes['role'] == 'hospital') {
        return node;
      }
    }
    return null;
  ...
  ChatGraphNode? _findPropertyInGraph(ChatEntityGraph graph) {
    for (final node in graph.nodes) {
      if (node.attributes['kind'] == 'property') return node;
    }
    return null;
  }
