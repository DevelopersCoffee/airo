...
/// Graph-workflow adapter for the Hospital Recovery built-in add-on.
class HospitalRecoveryGraphAdapter implements GraphWorkflowAddonAdapter {
  ...
  bool accepts(GraphIngestContext input) {
    if (_pending.looksHospital(input.text)) return true;
    if (_facts.looksHospitalStay(input.text)) return true;
    return _facts.extractMedicalSurgery(input.text).facts.isNotEmpty;
  }
  ...
  Future<EntityGraphPatch> extract(GraphIngestContext input) async {
    ...
    final after = _linker.ingestHospitalDomain(before, input.text);
  ...
  Iterable<WorkflowProjection> project(EntityGraph graph) {
    ...
    return chatGraph.nodes
        .where((node) => node.attributes['kind'] == 'hospital_stay')
        .map((stay) => _toProjection(
          LegacyWorkflowProjection.hospital(chatGraph, stay),
          chatGraph,
        ));
  }
  ...
  PendingAssessment assessPending(
    ...
    final journey = LegacyWorkflowProjection.hospital(chatGraph, chatNode);
    return PendingAssessment(
      identity: identity,
      subjectNodeId: projection.subjectNodeId,
