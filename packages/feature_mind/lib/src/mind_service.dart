...
class MindService {
  ...
  /// File ASR followed by reconciliation with live baseline segments
  /// (`ADR-0025` §3.1 live + refine).
  Stream<MindProgress> processWithRefine({
    required String wavPath,
    required String title,
    required List<TranscriptSegment> baselineSegments,
    String? language,
  }) async* {
    var progress = const MindProgress(stage: MindStage.transcribing);
    yield progress;
