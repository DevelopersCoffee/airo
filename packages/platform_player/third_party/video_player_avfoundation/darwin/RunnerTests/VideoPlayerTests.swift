...
private final class NotificationObjectCapture: @unchecked Sendable {
  nonisolated(unsafe) var object: AnyObject?
}
...
@MainActor struct VideoPlayerTests {
  ...
  @Test func texturePlayerPublishesPlayerLayerLifecycleForPictureInPicture() throws {
    let availableCapture = NotificationObjectCapture()
    let unavailableCapture = NotificationObjectCapture()
    let center = NotificationCenter.default
    let availableObserver = center.addObserver(
      forName: Notification.Name("AiroPlayerLayerAvailable"),
      object: nil,
      queue: nil
    ) { notification in
      availableCapture.object = notification.object as AnyObject?
    }
    let unavailableObserver = center.addObserver(
      forName: Notification.Name("AiroPlayerLayerUnavailable"),
      object: nil,
      queue: nil
    ) { notification in
      unavailableCapture.object = notification.object as AnyObject?
    }
    defer {
      center.removeObserver(availableObserver)
      center.removeObserver(unavailableObserver)
    }
    ...
    #expect(availableCapture.object === player.playerLayer)
    #expect(unavailableCapture.object == nil)

    var error: FlutterError?
    player.disposeWithError(&error)

    #expect(error == nil)
    #expect(unavailableCapture.object === player.playerLayer)
  }
