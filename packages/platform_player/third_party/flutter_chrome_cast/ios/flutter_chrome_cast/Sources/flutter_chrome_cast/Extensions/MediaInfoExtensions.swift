...
extension GCKMediaInformation{
    static func fromMap(_ arguments: Dictionary<String, Any> ) -> GCKMediaInformation? {
        ...
        // GCKMediaInformationBuilder.init(contentURL:) leaves contentID nil.
        // Some receivers (including Default Media Receiver on some TVs) key
        // playback off contentID, not contentURL, so without this the
        // receiver starts a media session but never has anything to fetch.
        builder.contentID = (arguments["contentID"] as? String) ?? contentUrlString
