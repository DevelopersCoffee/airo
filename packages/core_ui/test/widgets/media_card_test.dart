...
void main() {
  ...
  testWidgets('live variant shows LIVE badge', (tester) async {
    await tester.pumpWidget(
      _host(const MediaCard(name: 'Aaj Tak', variant: MediaCardVariant.live)),
    );
  ...
  testWidgets('variant dimensions are pinned', (tester) async {
    Future<(double, double)> dimensionsOf(MediaCardVariant variant) async {
      await tester.pumpWidget(
        _host(
          Row(
            children: [MediaCard(name: 'A', variant: variant)],
          ),
        ),
      );
    ...
    final (standardWidth, standardHeight) = await dimensionsOf(
    ...
    final (compactWidth, compactHeight) = await dimensionsOf(
  ...
  test('railHeightFor derives from each variant\'s thumbnail height', () {
    expect(MediaCard.railHeightFor(MediaCardVariant.compact), 84 + 65);
    expect(MediaCard.railHeightFor(MediaCardVariant.standard), 104 + 65);
    expect(MediaCard.railHeightFor(MediaCardVariant.live), 104 + 65);
    expect(MediaCard.railHeightFor(MediaCardVariant.hero), 180 + 65);
  });
}
