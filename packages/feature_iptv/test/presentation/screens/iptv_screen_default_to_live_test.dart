...
void main() {
  ...
  testWidgets('tapping Cancel then a late-arriving matching channel does not '
      'yank the user into fullscreen playback', (tester) async {
    ...
    await tester.pumpWidget(
      ProviderScope(
        overrides: [
          iptvChannelsProvider.overrideWith((ref) => channelsCompleter.future),
          recentlyWatchedChannelsProvider.overrideWith((ref) async => const []),
          streamingStateProvider.overrideWith(
            (ref) => Stream.value(
              StreamingState(
